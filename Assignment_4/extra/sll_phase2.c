/* 
	...
	and singly linked lists to perform the transformation.
...
void encode_rl(charval_t *, charval_t **);
void encode_ascii(charval_t *, charval_t **);

void decode(unsigned char *buffer);
void decode_ascii(unsigned char *, charval_t **);
void decode_rl(charval_t *, charval_t **);
void decode_mtf(charval_t *, charval_t **);
...
int main(int argc, char *argv[]) {
	...
	printf("SINGLY LINKED run time: %.3f sec\n", CPU_time);
...
/* Encodes a string of text by performing move-to-front
	...
	charval_t *lookup_list = NULL;
	charval_t *mtf_encoding = NULL;
	charval_t *rl_encoding = NULL;
	charval_t *ascii_encoding = NULL;

	encode_mtf(buffer, &lookup_list, &mtf_encoding);
	encode_rl(mtf_encoding, &rl_encoding);
	encode_ascii(rl_encoding, &ascii_encoding);
	write_to_file(ascii_encoding);

	/* Free the memory allocated by each list */
	delete_list(lookup_list);
	delete_list(mtf_encoding);
	delete_list(rl_encoding);
	delete_list(ascii_encoding);

}

void encode_mtf(unsigned char *buffer, charval_t **lookup_list, charval_t **mtf_encoding){
/* Performs move-to-front encoding on stream of input text */
	...
	for( ; *buffer; buffer++){
		...
		/* When the character is in the lookup list */
			...
			temp_char = peek_at(*lookup_list, index);
			switch_char = new_charval(temp_char->c, NOT_DIGIT);

			/* Remove node from end of the list and add to front */
			*lookup_list = remove_at(*lookup_list, index);
			*lookup_list = add_front(*lookup_list, switch_char);
...
void encode_rl(charval_t *mtf_encoding, charval_t **rl_encoding){
	...
	while(rl_char != NULL){
		...
		if((rl_char->flag == 1) && rl_char->c == 1){
			...
			if(duplicate_counter >= 3){
				...
				*rl_encoding = add_end(*rl_encoding, temp_char);
...
void encode_ascii(charval_t *rl_encoding, charval_t **ascii_encoding){
/* Performs a modified ascii encoding where characters 'flagged' as
	...
	for( ; rl_encoding != NULL; rl_encoding=rl_encoding->next){
		if(rl_encoding->flag == 1){
			...
			*ascii_encoding = add_end(*ascii_encoding, temp_char);
		} else {
...
/* Decodes a stream of text by decoding the modified ascii encoding, 
	...
	charval_t *ascii_decode = NULL; 
	charval_t *rl_decode = NULL;
	charval_t *mtf_decode = NULL;

	decode_ascii(buffer, &ascii_decode);
	decode_rl(ascii_decode, &rl_decode);
	decode_mtf(rl_decode, &mtf_decode);
	write_to_file(mtf_decode);

	/* Delete all lists to avoid any memory leaks */
	delete_list(ascii_decode);
	delete_list(rl_decode);
	delete_list(mtf_decode);

}

void decode_ascii(unsigned char *buffer, charval_t **ascii_decode){
/* Decodes an incoming stream of text by taking all characters whose
	...
	for( ; *buffer; buffer++){
		if(*buffer >= 128){
			...
			*ascii_decode = add_end(*ascii_decode, temp_char);
		} else {
			temp_char = new_charval(*buffer, NOT_DIGIT);
			*ascii_decode = add_end(*ascii_decode, temp_char);
		}
...
void decode_rl(charval_t *ascii_decode, charval_t **rl_decode){
	...
	while(ascii_decode != NULL){
		...
		if(ascii_decode->flag==1 && ascii_decode->c==0){
			...
			for(dups_count=0; dups_count < num_dups; dups_count++){
				temp_char = new_charval(1, IS_DIGIT);
				*rl_decode = add_end(*rl_decode, temp_char);
			}
...
void decode_mtf(charval_t *rl_decoding, charval_t **mtf_decoding){
	...
	while(rl_decoding != NULL){
		...
		if((rl_decoding->flag == 1) && (rl_decoding->next != NULL)){
			...
			if(rl_decoding->next->flag != 1){
				...
				lookup_list = add_end(lookup_list, lookup_char);
				*mtf_decoding = add_end(*mtf_decoding, decoded_char);
				...
				lookup_char = peek_at(lookup_list, index);
				...
				lookup_list = remove_at(lookup_list, index);
				lookup_list = add_front(lookup_list, move_char);
				*mtf_decoding = add_end(*mtf_decoding, decoded_char);
